...
pub async fn archive_column(
    ...
    let _ = db
        .log_activity(
            ...
            "column_archived",
            Some(
                &serde_json::json!({"column_name": &col_name, "task_count": task_count})
                    .to_string(),
            ),
        )
        .await;
    Ok(Json(
        serde_json::json!({ "archived": true, "task_count": task_count }),
    ))
}
...
pub async fn unarchive_column(
    ...
    let _ = db
        ...
        .await;
    Ok(Json(
        serde_json::json!({ "unarchived": true, "task_count": task_count }),
    ))
}
...
pub async fn list_archived(
    ...
    Ok(Json(
        serde_json::json!({ "tasks": tasks, "columns": columns }),
    ))
}
