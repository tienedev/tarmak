...
1. Resolve the board if not already cached:
   ...
   - `board_query` with `board_id: "list"` → find matching board (case-insensitive)
   - Cache column IDs via `board_query` with `board_id: <board_id>`, `scope: "columns"`
2. `board_query` with `board_id: <board_id>`, `scope: "tasks"` to get all tasks
3. Filter for tasks NOT in the "Done" column (compare each task's `column_id` against cached `done_id`)
